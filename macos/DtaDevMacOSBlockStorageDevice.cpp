...
bool DtaDevMacOSBlockStorageDevice::identify(DTA_DEVICE_INFO& disk_info ) {
    return BlockStorageDeviceUpdate(handleDriverService(osDeviceHandle), disk_info);
}

uint8_t DtaDevMacOSBlockStorageDevice::discovery0(DTA_DEVICE_INFO& disk_info ) {
    return BlockStorageDeviceUpdate(handleDriverService(osDeviceHandle), disk_info) ? 0x00 : 0xff;
}
