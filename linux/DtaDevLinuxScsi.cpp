...
bool OSSpecificInitialization(const char * devref,
  ...
  r = ioctl(handleDescriptor(osDeviceHandle), BLKGETSIZE64, &disk_info.devSize);
  if (r < 0) {
    errno = -r;
...
DtaDevLinuxScsi::getDtaDevLinuxScsi(const char * devref, DTA_DEVICE_INFO & di) {
  ...
  if (!OSSpecificInitialization(devref, osDeviceHandle, interfaceDeviceIdentification, di)) {
    LOG(E) << " Device " << devref << " failed OS specific initialization?! -- file handle " << handleDescriptor(osDeviceHandle);
