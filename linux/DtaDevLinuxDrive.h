...
public:
  ...
  virtual bool isOpen(void) {return base_type::isOpen() &&
                                    (fcntl(handleDescriptor(osDeviceHandle), F_GETFL) != -1 || errno != EBADF);}


  static
  OSDEVICEHANDLE openAndCheckDeviceHandle(const char * devref);

  void closeDrive();

  static
  OSDEVICEHANDLE openDeviceHandle(const char* devref);
